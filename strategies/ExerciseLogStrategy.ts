...
export class ExerciseLogStrategy implements LogStrategy<RawExerciseData> {
    async analyze(text: string, options?: ExerciseStrategyOptions): Promise<RawExerciseData> {
        const result = await analyzeExerciseInput(text, options?.image);
        // Ensure the strategy always returns a consistent shape, even if the service returns a single object
